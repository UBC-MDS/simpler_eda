...
def test_input_type():
    ...
    try:
        corr_map(["a", "b", "c"], ["Horsepower", "Displacement", "Cylinders", "Acceleration"])
    ...
    try:
        ...
        assert str(err) == "The correlation method should be 'pearson', 'kendall',  or'spearman'"

    try:
        corr_map(df, ["Horsepower", "Displacement", "Cylinders", "Acceleration"], color_scheme=["blueyellow"])
        
    except Exception as err:
        assert str(err) == "The color scheme should be given as a string"

    try:
        corr_map(df, ["Horsepower", "Displacement", "Cylinders", "Acceleration"], plot_width="200")
        
    except Exception as err:
        assert str(err) == "The plot_width should be given as an integer"

    try:
        corr_map(df, ["Horsepower", "Displacement", "Cylinders", "Acceleration"], plot_height="200")
        
    except Exception as err:
        assert str(err) == "The plot_height should be given as an integer"

    try:
        corr_map(df, ["Horsepower", "Displacement", "Cylinders", "Acceleration"], title=200)
...
def test_corr_map():
    ...
    assert out.encoding.y.shorthand == "level_1", 'The level 1 should be mapped to the y axis'
    assert out.mark.type == "rect", 'the plot type (mark) should be a rect plot (heatmap)'
    assert out.encoding.color.scale.scheme == 'blueorange', "the color scheme should blueorange (default) or the inputted color scheme"
    assert out.title == "Correlation Map", "The title should be Correlation Map or the given value"